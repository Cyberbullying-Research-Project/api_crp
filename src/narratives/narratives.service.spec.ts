...
describe('NarrativesService', () => {
    ...
    describe('findAll', () => {
        it('should return an array of narratives', async () => {
            ...
            jest.spyOn(model, 'find').mockImplementation(() => ({
                limit: ()=> ({skip: jest.fn().mockResolvedValue([mockNarrative])})
            } as any));
    ...
    describe('findById', () => {
        ...
        it('should throw an error if ID is invalid', async () => {
            ...
            await expect(narrativeService.getOne(id)).rejects.toThrow(BadRequestException);
        ...
        it('should throw NotFoundExeption if narrative is not found', async () => {
            ...
            await expect(narrativeService.getOne(mockNarrative._id)).rejects.toThrow(NotFoundException);
    ...
    describe('updateById', () => {
        it('should update a narrative', async () => {
            const updateNarrative = {...mockNarrative, title: 'Updated title'} as UpdateNarrativeDTO;
            const resource = { title: 'Update post'};

            jest.spyOn(model, 'findByIdAndUpdate').mockResolvedValue(updateNarrative);

            const result = await narrativeService.updateById(mockNarrative._id, updateNarrative as UpdateNarrativeDTO);

            expect(model.findByIdAndUpdate).toHaveBeenCalledWith(mockNarrative._id, updateNarrative, {
                new: true,
                runValidators: true
            });
